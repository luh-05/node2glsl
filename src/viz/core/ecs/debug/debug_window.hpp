#include "../../geometry/pch.h"
#include "../ecs.hpp"

namespace ntg::viz {
class DebugGui {
public:
  using Entity = ntg::viz::entity_t;
  using Crd = ntg::viz::Coordinator;
...
#define ImColorGrey ImVec4(.4f, .4f, .4f, 1.0f)
  ...
  inline void removeMonitoredInt(const char *name) {
    if (monitoredVariables.ints.find(name) == monitoredVariables.ints.end())
      return;
    monitoredVariables.ints.erase(name);
  }
  inline void removeMonitoredFloat(const char *name) {
    if (monitoredVariables.floats.find(name) == monitoredVariables.floats.end())
      return;
    monitoredVariables.floats.erase(name);
  }
  inline void removeMonitoredVec2(const char *name) {
    if (monitoredVariables.vec2s.find(name) == monitoredVariables.vec2s.end())
      return;
    monitoredVariables.vec2s.erase(name);
  }
  inline void removeMonitoredVec3(const char *name) {
    if (monitoredVariables.vec3s.find(name) == monitoredVariables.vec3s.end())
      return;
    monitoredVariables.vec3s.erase(name);
  }
  inline void removeMonitoredVec4(const char *name) {
    if (monitoredVariables.vec4s.find(name) == monitoredVariables.vec4s.end())
      return;
    monitoredVariables.vec4s.erase(name);
  }
  inline void removeMonitoredVolInt(const char *name) {
    if (monitoredVariables.volInts.find(name) ==
        monitoredVariables.volInts.end())
      return;
    monitoredVariables.volInts.erase(name);
  }
  inline void removeMonitoredVolFloat(const char *name) {
    if (monitoredVariables.volFloats.find(name) ==
        monitoredVariables.volFloats.end())
      return;
    monitoredVariables.volFloats.erase(name);
  }
  inline void removeMonitoredVolVec2(const char *name) {
    if (monitoredVariables.volVec2s.find(name) ==
        monitoredVariables.volVec2s.end())
      return;
    monitoredVariables.volVec2s.erase(name);
  }
  inline void removeMonitoredVolVec3(const char *name) {
    if (monitoredVariables.volVec3s.find(name) ==
        monitoredVariables.volVec3s.end())
      return;
    monitoredVariables.volVec3s.erase(name);
  }
  inline void removeMonitoredVolVec4(const char *name) {
    if (monitoredVariables.volVec4s.find(name) ==
        monitoredVariables.volVec4s.end())
      return;
    monitoredVariables.volVec4s.erase(name);
  }

  void addManagedVariable(
      const char *name,
      std::vector<std::tuple<void *, std::string, ManagedVariableDataType>>
          data);

  void showMenuBar();
  static void displayMaterial(Ths::Vk::Material &material,
                              [[maybe_unused]] Entity e,
                              [[maybe_unused]] Crd *crd);
  static void displayTransform(Ths::Vk::Transform &transform,
                               [[maybe_unused]] Entity e,
                               [[maybe_unused]] Crd *crd);
  static void displayMesh(Ths::Vk::Mesh &mesh, [[maybe_unused]] Entity e,
                          [[maybe_unused]] Crd *crd);
  static void displayObjectContext(Ths::Vk::OContext &context,
                                   [[maybe_unused]] Entity e,
                                   [[maybe_unused]] Crd *crd);
  template <typename T>
  static void displayComponent(Crd *crd, bool condition, const char *treeName,
                               Entity e,
                               void (*displayFunc)(T &, Entity, Crd *));
